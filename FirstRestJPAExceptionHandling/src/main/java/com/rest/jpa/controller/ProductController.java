...
import com.rest.jpa.exception.InvalidException;
...
public class ProductController {
	...
	@GetMapping("/test")
	public String message(String str) throws InvalidException, Exception{
		str="abc";
		if(str==null) {
			throw new NullPointerException("Value not Provided");
		}
		boolean isNum = str.chars().anyMatch(x -> Character.isDigit(x));
		if(isNum) {
			throw new InvalidException("Expected String");
		}
		if(str.length()<8) {
			throw new Exception("Length too Short");
		}
		else
			return "Hello Harithik";
	}
